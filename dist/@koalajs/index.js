...
const combineProviders = (providers) => {
  ...
  return ({ children }) => {
    return providers.reduceRight((acc, Provider) => {
      ...
      return /* @__PURE__ */ jsxRuntimeExports.jsx(Provider, { children: acc });
    }, children);
  };
};

const createState = ({ state = {}, actions = {} }) => {
  ...
  const reducer = (state2, action) => {
    ...
    if (typeof actionFn === "function") {
      return actionFn(state2, ...action.payload || []);
    }
    return state2;
  };
  const Provider = ({ children }) => {
    const [contextState, dispatch] = useReducer(reducer, state);
    const boundActions = require$$0.useMemo(() => {
      return Object.keys(actions).reduce((acc, type) => {
        acc[type] = (...payload) => dispatch({ type, payload });
        return acc;
      }, {});
    }, [actions]);
    return /* @__PURE__ */ jsxRuntimeExports.jsx(Context.Provider, { value: { state: contextState, actions: boundActions }, children });
  };
  const useStateContext = () => {
    ...
    const { state: state2, actions: actions2 } = context;
    if (state2 === null || state2 === undefined) {
      throw new Error("State cannot be null or undefined");
    }
    if (actions2 === null || actions2 === undefined) {
...
const createSelectiveState = ({ state, actions }) => {
  ...
  const useSelectiveContext = (selector) => {
    ...
    const selectedState = selector ? context.getValue(selector) : context.getValue();
...
export { addClass, combineProviders, createElement, createSelectiveState, createState, debounce, deepClone, getElement, mergeState, removeClass, safeGet, setAttributes, throttle, toggleClass };
